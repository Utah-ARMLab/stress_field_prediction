        ...
        for i in range(0,50):     # 50 time steps. Takes ~0.40 mins to process
            ...
            for combined_pc in combined_pcs:
                ...
                processed_data = {"combined_pc": combined_pc.transpose(1,0),
                                "stress_log": stress, "occupancy": occupancy,                                    
                                "query_points": query_points, "object_name": object_name, "grasp_idx": k}
            ...
            sampled_points = sample_points_bounding_box(trimesh.PointCloud(full_pc), round(num_query_pts*1.2), scales=[1.5]*3) 
            ...
            query_points = sampled_points[:num_query_pts]
            stress = -4 * np.ones(outside_mesh_idxs.shape[0])
            occupancy = np.zeros(stress.shape)
