...
from model import StressNetSDF, StressNetOccupancyOnly

mgn_dataset_main_path = "/home/baothach/shape_servo_data/stress_field_prediction/mgn_dataset"
gripper_pc_recording_path = os.path.join(mgn_dataset_main_path, "filtered_data")
...
model.load_state_dict(torch.load("/home/baothach/shape_servo_data/stress_field_prediction/mgn_dataset/weights/new_sdf_data/ellipsoid03-p1-sdf/epoch 100"))
model.eval()
...
for idx, file_name in enumerate(["ellipsoid03-p1"]):
    ...
    for k in range(0,100):    # 100 grasp poses
        ...
        for i in range(0,50):     # 50 time steps. Takes ~0.40 mins to process
            ...
            occupied_idxs = np.where(pred_occupancy >= -0.003)[0]
            ...
            open3d.visualization.draw_geometries([pcd.translate((0.09,0,0)), pcd_gt])
