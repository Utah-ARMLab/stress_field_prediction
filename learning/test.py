...
model.load_state_dict(torch.load("/home/baothach/shape_servo_data/stress_field_prediction/6polygon/varying_stiffness/weights/all_6polygon_open_gripper_new_2/epoch 62")) # _new
model.eval()
...
for idx, file_name in enumerate([f"6polygon0{j}" for j in [4]]):
    ...
    for k in range(0,100):    # 100 grasp poses 32, 7, 8(doesnt work),
        ...
        ### Sample query points
        query = sample_points_bounding_box(trimesh.PointCloud(partial_pc.squeeze()), num_query_pts, scales=[2.5]*3)  # shape (num_query_pts,3) 
        ...
        # for force_idx in range(0,25,5):
            ...
            if not create_media:            
                ...
                if use_open_gripper:
                    translation = (-0.07,0.00,0)
                    open3d.visualization.draw_geometries([pcd.translate(translation), deepcopy(pcd_gripper).translate(translation), 
                                                        deepcopy(pcd_gripper_open).translate(translation), 
