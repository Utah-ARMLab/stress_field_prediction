...
force_levels = np.arange(1, 15.25, 0.25)

for object_name in OBJECT_NAMES:
    for i in range(*grasp_idx_bounds):        
        print(f"Grasp {i} started. Time passed: {timeit.default_timer() - start_time}")
        ...
        for force in force_levels:
            ...
            for idx in selected_idxs:
                ...
                for pc in partial_pcs:
                    combined_pc = np.concatenate((pc, gripper_pc), axis=0)

                    # Save data
                    processed_data = {"combined_pc": combined_pc.transpose(1,0),\
                                    "stress": stress, "stress_log": stress_log, \
                                    "occupancy": 1, "outside": False, "inside": False, \
                                    "query_point": query_point, "partial_pc": pc.transpose(1,0), "object_name": object_name}
            ...
            sampled_points, outside_mesh_idxs = sample_and_compute_signed_distance(tri_indices, full_pc, \
                                            ...
                                            num_pts=num_query_pts, scales=(2, 2, 4), vis=visualization, seed=None, verbose=False)      
            ...
            for idx in outside_mesh_idxs:
                ...
                for pc in partial_pcs:
                    ...
                    processed_data = {"combined_pc": combined_pc.transpose(1,0),\
                                    ...
                                    "occupancy": 0, "outside": True, "inside": False, \
                                    "query_point": query_point, "partial_pc": pc.transpose(1,0), "object_name": object_name}
